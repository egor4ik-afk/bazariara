        ...
        <div className="bg-cream-100 min-h-screen text-ink-900">
            <main className="container mx-auto px-4 py-8 sm:px-6 lg:px-8">
                ...
                {cartItems.length === 0 && pendingRemoval.length === 0 ? (
                    <div className="text-center bg-surface rounded-2xl p-12 border border-ink-200 shadow-card">
                        <p className="text-2xl font-semibold mb-6 text-ink-700">{t('cart.empty')}</p>
                        <Link href="/" className="bg-brand-600 text-on-brand font-bold py-3 px-8 rounded-full hover:bg-brand-500 transition-all duration-300 transform hover:scale-105 shadow-lg shadow-brand-600/30 hover:shadow-xl hover:shadow-brand-500/30">
                            {t('cart.startShopping')}
                        </Link>
                    </div>
                ) : (
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
                        <div className="lg:col-span-2 bg-surface rounded-xl shadow-lg backdrop-blur-sm">
                                    ...
                                    <li key={key} className="flex flex-col sm:flex-row justify-between items-center p-5 gap-4">
                                        ...
                                        <div className="flex items-center justify-end gap-3 sm:gap-4 self-end sm:self-center w-full sm:w-auto">
                                            {pendingRemoval.includes(key) ? (
                                                <div className="relative h-10 font-medium rounded-xl bg-surface/80 border border-transparent shadow-inner flex items-center justify-center overflow-hidden w-[116px]">
                        ...
                        {/* Order Summary */}
                        <div className="lg:col-span-1 bg-surface/60 rounded-xl shadow-xl p-6 backdrop-blur-sm top-20 sticky">
                           <h2 className="text-2xl font-bold border-b border-ink-200 pb-4 mb-4">{t('cart.orderSummary')}</h2>
                            ...
                            <div className="mt-8 flex flex-col gap-4">
                                {isCheckoutDisabled ? (
                                    <>
                                        <Link href="/"
                                            className="w-full text-center font-bold py-3 px-6 rounded-lg transition-all duration-300 transform hover:scale-105 shadow-lg bg-brand-600 text-on-brand hover:bg-brand-500 shadow-brand-600/30 hover:shadow-xl"
                                        >
                                    ...
                                    <button 
                                        ...
                                        className={`w-full text-center font-bold py-3 px-6 rounded-lg transition-all duration-300 transform hover:scale-105 shadow-lg flex items-center justify-center ${pendingRemoval.length > 0 || isNavigating ? 'bg-ink-200 text-ink-600 cursor-not-allowed' : 'bg-brand-600 text-on-brand hover:bg-brand-500 shadow-brand-600/30 hover:shadow-xl'}`}
