...
export async function PATCH(req: NextRequest, { params }: { params: Params }) {
  // Любая ошибка — JSON с причиной. Раньше необработанное исключение
  // давало 500 без текста, и форма показывала безликое «Ошибка сохранения».
  try {
    ...
    maybe('sub_category',    'sub_category', v => String(v || '').trim() || null);
    ...
    // trim: пустое или пробельное название — не подкатегория. Иначе
    // в справочник попадала запись с пустым ключом.
    const subName = String(body.sub_category || '').trim();
    if (subName && category_key_for_subcategories) {
      const subKey = subName.toLowerCase().replace(/\s+/g, '-');
      await sql`
        INSERT INTO subcategories (category_key, key, name, name_en, name_ka, image_url)
        VALUES (${category_key_for_subcategories}, ${subKey}, ${subName},
