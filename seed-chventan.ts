...
async function main() {
  ...
  // ── 7. Диагностика невидимых товаров.
  //       Витрина фильтрует WHERE source = 'gorgia' AND image_url IS NOT NULL
  //       (app/actions.ts). Товар с source = 'market' — а это ЗНАЧЕНИЕ ПО
  //       УМОЛЧАНИЮ в schema.prisma — в каталоге не появится никогда.
  //       Именно так пропадает вино, если его заводили скриптом без source.
  const invisible = await sql`
    SELECT id, external_id, name_ru, source, image_url IS NULL AS no_image
    FROM products
    WHERE source <> 'gorgia' OR image_url IS NULL OR image_url = ''
  `;
  if (invisible.length > 0) {
    console.log(`\n⚠️  Не попадут на витрину — ${invisible.length} шт:`);
    for (const r of invisible) {
      const why = r.source !== 'gorgia' ? `source='${r.source}'` : 'нет image_url';
      console.log(`   #${r.id} ${r.name_ru} — ${why}`);
    }
    console.log('   Чиню…');
    await sql`
      UPDATE products SET source = 'gorgia', updated_at = NOW() WHERE source <> 'gorgia'
    `;
    await sql`
      UPDATE products SET image_url = '/placeholder-product.svg', updated_at = NOW()
      WHERE image_url IS NULL OR image_url = ''
